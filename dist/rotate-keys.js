...
function parseArgs(argv) {
  const opts = { targetEnv: "all", invalidateKeys: true, init: null };
  const args = argv.slice(2);
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg === "--env") {
      const val = args[++i];
      if (!val) (0, logger_1.err)("--env requires a value");
      ...
      (0, logger_1.err)(
        `Unknown option: ${arg}. Run 'rotate-keys --help' for usage.`,
      );
    }
  }
  return opts;
...
// ─── Prerequisites ────────────────────────────────────────────────────────────
function checkPrereqs(needsGcloud) {
  const missing = [];
  if (!(0, subprocess_1.commandExists)("vercel")) missing.push("vercel");
...
async function rotateFirebase(opts, client, tempDir) {
  ...
  for (const vercelEnv of targetEnvs(opts.targetEnv)) {
    ...
    (0, logger_1.log)(
      `  [${vercelEnv}] New key ID: ${newSaJson.private_key_id}`,
    );
  ...
  if (!rotatedAny)
    (0, logger_1.err)(
      "No Firebase keys rotated — check --env and project configuration",
    );
...
async function rotateSentry(opts, client) {
  ...
  if (!dsnKeyName)
    (0, logger_1.err)(
      "Could not find SENTRY_DSN or NEXT_PUBLIC_SENTRY_DSN in Vercel",
    );
  ...
  const newKey = await sentryRequest(
...
async function triggerAndWaitRedeployments(opts, client) {
  ...
  for (const vercelEnv of targetEnvs(opts.targetEnv)) {
    if (vercelEnv === "development") {
      (0, logger_1.log)(
        `  [${vercelEnv}] No remote deployment target — skipping`,
      );
      continue;
    }
  ...
  (0, logger_1.log)(
    `Waiting for ${deploymentIds.length} deployment(s) to finish...`,
  );
...
async function invalidateFirebaseKeys(client, fp) {
  ...
  for (const [saEmail, gcpProject] of saPairs) {
    if (unsweepable.has(saEmail)) {
      ...
      (0, logger_1.warn)(
        "  Rotate all environments first, then re-run to sweep old keys.",
      );
      continue;
    }
    ...
    if (deleted === 0) (0, logger_1.log)(`  No stray keys for ${saEmail}.`);
    else (0, logger_1.log)(`  Deleted ${deleted} stray key(s) for ${saEmail}.`);
  }
}
async function invalidateSentryKey(oldKeyId, org, project) {
  if (!oldKeyId) {
    (0, logger_1.warn)(
      "No old Sentry key ID recorded — skipping Sentry invalidation",
    );
    return;
  }
...
async function initFirebase(opts, client, tempDir) {
  (0, logger_1.log)("Initializing Firebase service account keys...");
  const saEmail = process.env.FIREBASE_SA_EMAIL;
  if (!saEmail)
    (0, logger_1.err)("FIREBASE_SA_EMAIL is required for --init firebase");
  const gcpProject = process.env.GCLOUD_PROJECT;
  if (!gcpProject)
    (0, logger_1.err)("GCLOUD_PROJECT is required for --init firebase");
  const currentEnvs = await client.listEnvVars();
  for (const vercelEnv of targetEnvs(opts.targetEnv)) {
    const keyFile = path.join(tempDir, `key-${vercelEnv}.json`);
    ...
    const newSaJson = JSON.parse(fs.readFileSync(keyFile, "utf-8"));
    (0, logger_1.log)(
      `  [${vercelEnv}] Created key ID: ${newSaJson.private_key_id}`,
    );
    await client.setEnvForTarget(
      "FIREBASE_SERVICE_ACCOUNT",
      JSON.stringify(newSaJson),
...
async function initSentry(opts, client) {
...
async function run(opts) {
  ...
  const client = new vercel_api_1.VercelClient(
